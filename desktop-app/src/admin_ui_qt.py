...
class ApiIntegrationFrameQt(QWidget):
    ...
    def _update_buttons_state(self):
        ...
        elif api_status == 'Отчет проверен':
            # Все кнопки остаются скрытыми, выводим финальное сообщение
            self._display_api_response("Проверка пройдена", "Работа с заказом в АПИ полностью завершена. Все отчеты приняты и проверены.")
...
class AdminWindowQt(QMainWindow):
    ...
    def __init__(self, user_info: dict):
        ...
        self.supply_notification_service = SupplyNotificationService(lambda: get_client_db_connection(self.user_info), lambda: self.user_info)
    ...
    def load_notification_files(self, notif_id, target_table=None):
        ...
        try:
            files = self.supply_notification_service.get_notification_files(notif_id)
