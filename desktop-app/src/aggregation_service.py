...
def create_bartender_views(user_info: Dict[str, Any], order_id: int) -> dict:
    ...
    try:
        ...
        with get_client_db_connection(user_info) as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            # --- ДЕБАГ: Оставляем только получение курсора и лог ---
            logging.debug(f"Получено соединение с БД и курсор для заказа ID: {order_id}")
            pass # Больше ничего не делаем
        
        # Временно возвращаем успех, чтобы проверить только этап подключения
        logging.info(f"Тест получения курсора для заказа №{order_id} успешно пройден.")
        return {"success": True, "message": f"Тест получения курсора для заказа №{order_id} успешно пройден."}
