...
class AdminWindow(tk.Tk):
    ...
    def _create_supply_notice_tab(self, parent_frame):
        ...
        # --- НОВАЯ ЛОГИКА: Используем grid для точного и предсказуемого выравнивания ---
        # Настраиваем колонки сетки, чтобы они растягивались пропорционально
        summary_header_frame.columnconfigure(0, weight=1) # Левая распорка
        summary_header_frame.columnconfigure(1, minsize=200) # Колонка для клиента
        for i in range(4):
            summary_header_frame.columnconfigure(i + 2, weight=2) # Колонки для дат
        summary_header_frame.columnconfigure(6, weight=1) # Правая распорка

        # Пустышка для выравнивания с колонкой "Клиент"
        ttk.Frame(summary_header_frame).grid(row=0, column=1)
        ...
        for i, day_label in enumerate(day_labels):
            ...
            # Размещаем в сетке
            date_header_block.grid(row=0, column=i + 2, sticky="ew")
