...
class ScanProcessor:
    ...
    def _add_to_unit(self, scanned_code):
        ...
        # --- НОВАЯ ЛОГИКА: Проверка вместимости набора ---
        if unit_type == 'set':
            # `set_capacity` должно быть передано в `order_info` из view
            set_capacity = self.order.get('set_capacity')
            # Вместимость набора - это количество ТОВАРОВ. Общее количество сканов = вместимость + 1 (код самого набора).
            # Поэтому блокируем сканирование, если количество УЖЕ отсканированных кодов равно или больше вместимость + 1.
            if set_capacity and len(current_items) >= (set_capacity + 1):
                return self._build_error_response(
                    f"Ошибка: Максимальное количество кодов ({set_capacity} товаров + 1 код набора) уже отсканировано. "
                    f"Завершите операцию с помощью управляющего кода."
                )
