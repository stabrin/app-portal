...
# Создаем корневой CA сертификат для клиентов
CA_CERT="$CERT_DIR/ca.crt"
if [ ! -f "$CA_CERT" ]; then
    echo "Создаем корневой CA сертификат для клиентов..."
    cp "$PG_CERT" "$CA_CERT"
...
openssl x509 -in "$PG_CERT" -text -noout | grep -A 10 "Subject Alternative Name"
